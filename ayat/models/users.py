...
class User(db.Model):
    ...
    country_name = db.Column(db.VARCHAR(60), nullable=False)
    ...
    def equals(self, user):
        if self.name == user.name \
                ...
                and self.country_name == user.country_name \
                and self.phone_number == user.phone_number \
                and self.profile_picture == user.profile_picture \
                and self.birth_date == user.birth_date \
                and self.gender == user.gender \
                and self.password == user.password \
                and self.registration_date == user.registration_date \
                and self.is_activated == user.is_activated \
                and self.type == user.type:
            return True
        return False
...
class Guardian(db.Model):
    ...
    def equals(self, guardian):
        if self.email == guardian.email and self.phone_number == self.phone_number:
            return True
        return False
...
class Permission(db.Model):
    ...
    permission_name = db.Column(db.VARCHAR(50), unique=True, nullable=False)
    ...
    def equals(self, permission):
        if self.permission_name == permission.permission_name:
            return True
        return False
